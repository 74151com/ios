...
``` pch
#define PrivacyAgreementBaseApiUrl  @"103.145.107.68" // @"chat.域名"
#define SecondChannelUrl @"http://103.145.107.68:8095" // @"http://api.域名:8095"
```
Tigase/personCenter(个人中心)/WH_Recharge_WHViewController.m 
``` m 
NSString *host = @"http://103.145.107.68"; // @"http://pay.域名";
NSString *callbackUrl = @"http://103.145.107.68:8092/pay/callbackUrl"; // @"http://api.域名/pay/callbackUrl";
```
Tigase/networkRequest(网络请求)/WH_VersionManageTool.m
``` m 
- (NSString *)getConfigDefault:(NSString *)url {
    NSString* defApiUrl = @"http://103.145.107.68:8092/config";
