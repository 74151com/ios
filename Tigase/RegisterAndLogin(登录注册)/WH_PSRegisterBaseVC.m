...
@interface WH_PSRegisterBaseVC ()<UITextFieldDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate,WH_JXActionSheet_WHVCDelegate,WH_JXCamera_WHVCDelegate, UITableViewDelegate, UITableViewDataSource,UIAlertViewDelegate,CLLocationManagerDelegate>
{
...
- (WH_RoundCornerCell *)getInfoCell:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        arrowImage.tag = 104; // Add this line to set the tag
        [cell.contentView addSubview:arrowImage];
    }
    UILabel *titleLabel = (UILabel *)[cell.contentView viewWithTag:101];
    UITextField *nickTextField = (UITextField *)[cell.contentView viewWithTag:10];
    UISegmentedControl *sexSegment = (UISegmentedControl *)[cell.contentView viewWithTag:103];
    ...
    sexSegment.hidden = YES;
    cityLabel.hidden = YES;
    arrowImage.hidden = YES;

    if (indexPath.row == 0) {
        titleLabel.text = Localized(@"JX_NickName");
        nickTextField.hidden = NO;
        nickTextField.text = _user.userNickname; // Add this line
        arrowImage.hidden = YES; // 昵称行不显示箭头
    } else if (indexPath.row == 1) {
        titleLabel.text = Localized(@"JX_Sex");
        sexSegment.hidden = NO;
        sexSegment.selectedSegmentIndex = [_user.sex boolValue];
        arrowImage.hidden = YES; // 性别行不显示箭头
    } else {
        titleLabel.text = Localized(@"JX_Address");
        cityLabel.hidden = NO;
        arrowImage.hidden = NO; // 居住地行显示箭头
    }
