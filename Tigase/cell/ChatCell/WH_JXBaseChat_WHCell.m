...
-(void)setCellData{    
    ...
    //用户等级
    WH_JXUserObject *userFriend = [[WH_JXUserObject alloc]init];
    userFriend = [userFriend getFriendWithUserId:self.msg.fromUserId];
    
    if ([roleData.vip integerValue] != 0||[userFriend.vip intValue]>0)  {
        ...
        if (roleData.vip.integerValue == 10) {
            ...
            if([userFriend.vip intValue]>0){
                imageName = [NSString stringWithFormat:@"grade_0%@", userFriend.vip];
            }else{
...
//    _msg.vip = [roleData.vip intValue];
    _isVip = [roleData.vip intValue];
    if (self.msg.isGroup) {
...
- (UIImageView *)gradeImgView {
    if (!_gradeImgView) {
        _gradeImgView = [[UIImageView alloc] initWithFrame:CGRectMake(self.headImage.width - 18, self.headImage.top - 8, 16, 16)];
    }
...
//#endif
    ...
    if(self.msg.isMySend){
        ...
        if ([_msg.type intValue] == kWCMessageTypeImage || [_msg.type intValue] == kWCMessageTypeVideo || [_msg.type intValue] == kWCMessageTypeLocation || [_msg.type intValue] == kWCMessageTypeRedPacket || [_msg.type intValue] == kWCMessageTypeRedPacketExclusive || [_msg.type intValue] == kWCMessageTypeFile || [_msg.type intValue] == kWCMessageTypeCard || [_msg.type intValue] == kWCMessageTypeLink || [_msg.type intValue] == kWCMessageTypeGif || [_msg.type intValue] == kWCMessageTypeShake || [_msg.type intValue] == kWCMessageTypeMergeRelay || [_msg.type intValue] == kWCMessageTypeShare || [_msg.type intValue] == kWCMessageTypeTransfer || [_msg.type intValue] == kWCMessageTypeNoteBook) {
            return;
        }
...
//            _readImage.frame = CGRectMake(JX_SCREEN_WIDTH-HEAD_SIZE-imageItemHeight-INSETS*2 + 40 - 20 - INSETS * .5, 20, _readImage.frame.size.width, _readImage.frame.size.height);
        ...
        if( [self.msg.isRead intValue] == 0){
            _readImage.image = [UIImage imageNamed:@"weidu"];
//            _readImage.image = nil;
        }else{
            _readImage.image = [UIImage imageNamed:@"yidu"];
...
- (void)creatLongPressItems
{
    ...
    //回复
    if (self.msg.isMySend) { }
        QBPopupMenuItem *item8 = [QBPopupMenuItem itemWithTitle:[NSString stringWithFormat:@"   %@   ", @"编辑"] target:self action:@selector(withEditAction)];
        [items addObject:item8];
   
  
    
    //收藏
    if ([self.msg.type intValue] == kWCMessageTypeImage || [self.msg.type intValue] == kWCMessageTypeVideo || [self.msg.type intValue] == kWCMessageTypeFile || [self.msg.type intValue] == kWCMessageTypeVoice || [self.msg.type intValue] == kWCMessageTypeText || [self.msg.type intValue] == kWCMessageTypeReply || [self.msg.type intValue] == kWCMessageTypeNoteBook) {
...
#pragma mark ---编辑
- (void)withEditAction {
    if ([self.chatCellDelegate respondsToSelector:@selector(chatCell:withEditIndexNum:)]) {
        [self.chatCellDelegate chatCell:self withEditIndexNum:self.indexNum];
    }
}
