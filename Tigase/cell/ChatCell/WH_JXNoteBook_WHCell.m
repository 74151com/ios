...
@implementation WH_JXNoteBook_WHCell
...
//    [_imageBackground release];
    ...
    _title.text = @"笔记"; Localized(@"JX_BusinessCard");
...
-(void)setCellData{
    ...
    NSArray *fileUrls = [self.msg.fileName componentsSeparatedByString:@"===="];
    
    NSArray *videoUrlsArr  = [self.msg.fileName componentsSeparatedByString:@"===="];
    NSArray *fileNameStr = [videoUrlsArr firstObject];
    videoUrlsArr = [fileNameStr mj_JSONObject];
    NSDictionary *dictDataImgVideo = [videoUrlsArr firstObject];
    
    NSString *oUrltype = [dictDataImgVideo objectForKey:@"oUrl"];
    if([oUrltype hasSuffix:@".mp4"]){
       [FileInfo getFirstImageFromVideo:oUrltype imageView:_cardHeadImage];
        
    }else if (isFileExist(oUrltype)){//判断是否是本地路径
        [FileInfo getFirstImageFromVideo:oUrltype imageView:_cardHeadImage];
    }else{
         [_cardHeadImage sd_setImageWithURL:[NSURL URLWithString:[dictDataImgVideo objectForKey:@"oUrl"]]];
    }
...
-(void)didTouch:(UIButton*)button{
    ...
    [g_notify postNotificationName:kCellShowNoteBookNotifaction object:self.msg];
}
