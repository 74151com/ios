...
//    [self setMaskLayer:_imageBackground];
    ...
    if(self.msg.isGroup){
        _nameLabel.text = self.msg.fileName.length > 0 ? self.msg.fileName : self.msg.isMySend ? [NSString stringWithFormat:@"%@%@",Localized(@"JX_TransferTo"),self.msg.toUserName] :[NSString stringWithFormat:@"%@%@",self.msg.fromUserName,Localized(@"JX_TransferTo")] ;
    }else{
