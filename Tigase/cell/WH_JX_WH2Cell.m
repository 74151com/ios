...
@synthesize bottomTitle,headImage,bage,userId;
...
//        _vipTitle = [UIFactory WH_create_WHLabelWith:CGRectMake(CGRectGetMaxX(self.lbTitle.frame)+2, CGRectGetMinY(_positionLabel.frame)+25, 50, 20) text:@"" font:sysFontWithSize(14) textColor:[UIColor whiteColor] backgroundColor:nil];
//         _vipTitle.layer.backgroundColor = [UIColor orangeColor].CGColor;
//        _vipTitle.layer.cornerRadius = 8;
//        _vipTitle.text = @"vip";
//        _vipTitle.textAlignment = NSTextAlignmentCenter;
//        _vipTitle.hidden = YES;
//        [_bgView addSubview:_vipTitle];
//        _vipTitle.textColor = [UIColor whiteColor]; HEXCOLOR(0x576b95);
...
//        [self.timeLabel release];
        ...
        _readImageView = [[WH_JXImageView alloc] initWithFrame:CGRectMake(JX_SCREEN_WIDTH-25-15-18, 40, 24, 17)];
        _readImageView.image = [UIImage imageNamed:@"weidu"];
        //_readImageView.hidden = YES;
        [_bgView addSubview:_readImageView];
...
//    title = [s retain];  _user;
    
    _title = s;
  
    if([s containsString:@"客服公众号"]){
//        self.lbTitle.textColor = RGB(251, 51, 51);
//        self.lbTitle.text = [NSString stringWithFormat:@"%@ (官方)",s];
//        self.lbTitle.textColor = THEMECOLOR;
//        title = [NSString stringWithFormat:@"%@ (官方)",s];
       
        NSMutableAttributedString *hintStringx=[[NSMutableAttributedString alloc]initWithString: [NSString stringWithFormat:@"%@ (%@)",s,@"官方"]];
        NSRange range2cx = [[hintStringx string]rangeOfString:@"官方"];
         [hintStringx addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:range2cx];
     //    [hintStringx addAttribute:NSFontAttributeName value: [UIFont systemFontOfSize:13.0 weight:UIFontWeightMedium] range:range2cx];
         self.lbTitle.attributedText = hintStringx;
        
    }else{
        self.lbTitle.textColor = RGB(51, 51, 51);
        self.lbTitle.text = s;
    }
...
-(void)setUser:(WH_JXUserObject *)user{
    ...
    _user = user;
    if([user.vip intValue]>0&&![user.userId isEqualToString:@"10000"]){
      //  _vipTitle.hidden = NO;
     // _vipTitle.text =  [NSString stringWithFormat:@"vip%d",[user.vip intValue]];
        
    }else{
       // _vipTitle.hidden = YES;
    }
   // _vipTitle.frame = CGRectMake(self.lbTitle.frame.origin.x + 50, CGRectGetMinY(_positionLabel.frame)-5, 40, 18);
 //   _vipTitle.center = CGPointMake(_positionLabel.center.x, 54 / 2);
}
