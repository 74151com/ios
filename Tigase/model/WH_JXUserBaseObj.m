...
-(id)init{
    ...
    if(self){
        ...
        self.vip = [NSNumber numberWithInt:0];;
    }
    return self;
}
...
-(BOOL)insert
{
    ...
    self.downloadTime = [NSDate dateWithTimeIntervalSince1970:0];
    NSString *insertStr=[NSString stringWithFormat:@"INSERT INTO '%@' ('userId','userNickname','remarkName','describe','role','createUserId','userDescription','userHead','roomFlag','category','timeCreate','newMsgs','status','userType','companyId','type','content','isMySend','roomId','timeSend','downloadTime','lastInput','showRead','showMember','allowSendCard','allowInviteFriend','allowUploadFile','allowConference','allowSpeakCourse','topTime','groupStatus','isOnLine','isOpenReadDel','isGroupSignIn','isSendRecipt','isDevice','chatRecordTimeOut','offlineNoPushMsg','isAtMe','talkTime','joinTime','clearChatTime','vip') VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",_tableName];
    NSString *crateroomDB = [NSString stringWithFormat:@"self.userId =%@self.userNickname= %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@                                                  %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@ %@",self.userId,self.userNickname,self.remarkName,self.describe,self.role,self.createUserId,self.userDescription,nil,self.roomFlag,self.category,self.timeCreate,self.msgsNew,self.status,self.userType,self.companyId,self.type,self.content,self.isMySend,self.roomId,self.timeSend,self.downloadTime,self.lastInput,self.showRead,self.showMember,self.allowSendCard,self.allowInviteFriend, self.allowUploadFile,self.allowConference,self.allowSpeakCourse,self.topTime,self.groupStatus,self.isOnLine,self.isOpenReadDel,self.isGroupSignIn,self.isSendRecipt,self.isDevice,self.chatRecordTimeOut,self.offlineNoPushMsg,self.isAtMe,self.talkTime,self.joinTime,self.clearChatTime,self.vip];
    
    NSLog(@"crateroomDb ===content %@= %@",crateroomDB,self.content);
    NSString *userIstr = [NSString stringWithFormat:@"%@",userId];
    if(userIstr.length>9&&self.content==nil ){
        NSString *device = userIstr;
        if ([device containsString:@"android"]||[device containsString:@"web"]||[device containsString:@"pc"]||[device containsString:@"mac"]) {
        }else{
            self.content = self.userNickname;
        }
        
        NSLog(@"content = %@-- userId = %@",self.content,userId);
    }
...
-(BOOL)haveTheUser
{
    ...
    if(b){
      ...
      //  user.topTime = [NSDate date];
        NSString* sql = [NSString stringWithFormat:@"update %@ set userNickname=?,content=?,status=?,timeSend=?,userType=?,vip=? where userId=?",_tableName];
        BOOL worked = [db executeUpdate:sql,self.userNickname,self.content,[NSNumber numberWithInt:2],self.timeSend,[NSNumber numberWithInt:2],self.vip,self.userId];
...
-(void)userFromDataset:(WH_JXUserBaseObj*)obj rs:(FMResultSet*)rs{
    ...
    obj.vip = [rs objectForColumnName:kUSER_Vip];
}
...
-(void)userFromDictionary:(WH_JXUserBaseObj*)obj dict:(NSDictionary*)aDic
{
    ...
    obj.vip = [aDic objectForKey:kUSER_Vip];
}
...
-(NSDictionary*)toDictionary
{
    ...
    if (self.vip) {
        [dic setObject:self.vip forKey:kUSER_Vip];
    }
    return dic;
}
