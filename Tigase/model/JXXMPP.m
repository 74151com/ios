...
//        return;
    ...
    if(![blackList containsObject:fromUserId]){ //排除黑名单，未对黑名单处理
        ...
        NSDictionary* resultObject = [body mj_JSONObject];
//        [resultParser release];
        if([[resultObject objectForKey:@"type"] intValue]==904){
            _removeDict =resultObject;
        }
        WH_JXMessageObject *msg=[[WH_JXMessageObject alloc] init];
        if([type isEqualToString:@"chat"] || [type isEqualToString:@"groupchat"]|| [[resultObject objectForKey:@"type"] intValue]==666){
            //创建message对象
            msg.messageId    = [messageId copy];
            //            msg.timeSend     = [NSDate date];//接收消息一律以本机时间为准，才不会乱序
            if(msg.messageId == nil)
            ...
            if (([type isEqualToString:@"chat"]||[msg.type intValue]==666) && msg.fromUserId && msg.toUserId && [msg.fromUserId isEqualToString:msg.toUserId]) {
                NSString *from = [message fromStr];
                NSRange range = [from rangeOfString:@"/"];
                NSString *device =@"";
                if([msg.type intValue]!=666){
                    device = [from substringFromIndex:range.location + 1];
                }
                if ([device isEqualToString:@"android"]) {
                    msg.fromUserId = ANDROID_USERID;
                    fromUserId  = ANDROID_USERID;
            ...
            NSString *userididdi = MY_USER_ID;
            NSString *userididdi2 = msg.fromUserId;
            if ([msg.fromUserName isKindOfClass:[NSString class]]) {
              //  if([msg.fromUserName length]>0 && [type isEqualToString:@"chat"] && ![msg.fromUserId isEqualToString:MY_USER_ID]){//保存陌生人信息：
               if([msg.fromUserName length]>0 &&[type isEqualToString:@"chat"] && ![msg.fromUserId isEqualToString:MY_USER_ID]){//保存陌生人信息： 笔记
                    [self saveFromUser:msg];
                }
            }
            ...
            if ([msg.type intValue] == kWCMessageTypeMultipleLogin) {
                ...
                if ((([type isEqualToString:@"groupchat"]||[msg.type intValue]==666)&&[msg.toUserId length]>8)||[msg.type intValue]==904) {
                    msg.isGroup = YES;
//                    if (x) {
//                        msg.isDelay = YES;
//                    }
...
//                [self sendMsgReceipt:message];//收到已读消息后，发回执，确认收到
                BOOL isHave = [msg haveTheMessage];
                BOOL inserted = NO;
                if ([type isEqualToString:@"chat"]||[msg.type intValue]==666) {
                    inserted = [msg insert:nil];
                }else {
                    msg.isGroup  = YES;
                    msg.toUserId = fromUserId;
...
//                    }
            ...
            if(msg.type != nil ){
                // 判断是否是编辑消息
                if ([msg.type intValue] == kWCMessageTypeWithEdit || [msg.type intValue] == kWCMessageTypeWithEditWithServer) {
...
//                        [self sendMsgReceipt:message];
                    ...
                    newMsg.toUserId = msg.toUserId;
                    newMsg.timeSend = msg.timeSend;
                    //  messageId      *    "ee29767693d5483a8268889812a3912f"
                    WH_JXMessageObject *msg1 = [newMsg getMsgWithMsgId:msg.messageId];
                    if (msg1 &&[msg1.type integerValue] != kWCMessageTypeRemind) {
                        [newMsg updateLastSend:UpdateLastSendType_None];
                        [g_notify postNotificationName:kXMPPMessageWithEdit_WHNotification object:newMsg];
                    }
                    [newMsg update];
                    msg = nil;
                    return;
...
//                        [self sendMsgReceipt:message];
                ...
                if((([type isEqualToString:@"chat"]||[msg.type intValue]==666))&&[msg.fromId length]<10){
                    //单聊发送回执：
                    if (![fromUserId isEqualToString:MY_USER_ID]) {
//                        [self sendMsgReceipt:message];
                    }
...
//                        }
                    ...
                    BOOL isRoomControlMsg = msg.isRoomControlMsg;
                    if(!isRoomControlMsg){
                        if([msg.type intValue]==120||[msg.type intValue]== kWCMessageTypeAudioMeetingJoin){
                          //  msg.toUserId = fromUserId;
                        }else{
...
//                    if(![msg.fromUserId isEqualToString:MY_USER_ID]){
                    ...
                    if(![msg insert:fromUserId]){ //在保存时检测MessageId是否已存在记录
                        if (isRoomControlMsg) { //
                            _msgDelete = msg;
                            if([[_removeDict objectForKey:@"type"] intValue ]==904){
                                [g_server getUser:[_removeDict objectForKey:@"fromUserId"] toView:self];
                            }
                            //移除当前群某用户所有的聊天记录
                            return;
                        }
                        ...
                        if([[_removeDict objectForKey:@"type"] intValue ]==904){
                            [g_server getUser:[_removeDict objectForKey:@"fromUserId"] toView:self];
//                             _msgDelete = msg;
//                            [msg deleteWithFromUser:_removeDict[@"toUserId"] roomId:msg.toUserId];
//                          [g_notify postNotificationName:@"kXMPPDeleteMsg_WHNotifaction" object:_removeDict userInfo:nil];//刷新聊天详情页
                        }
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if( [aDownload.action isEqualToString:wh_act_UserGet] ){
        ...
        if([[dict objectForKey:@"vip"] intValue]>0){
             ...
             [_msgDelete deleteWithFromUser:_removeDict[@"toUserId"] roomId:_msgDelete.toUserId];
...
//    user.vip = msg.vip;
 ...
 // and topTime > 0 and userId != %@ and timeSend > ifnull(clearChatTime,0) order by timeSend desc", FRIEND_CENTER_USERID];
//    [user release];
}
