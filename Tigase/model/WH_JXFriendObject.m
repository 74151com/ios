...
-(BOOL)insert{
    ...
    self.msgsNew = [NSNumber numberWithLong:1];
    return [super insert];
}
...
-(NSMutableArray*)seleectNewMsgUserId:(NSString*)sql
{
    NSMutableArray *resultArr=[[NSMutableArray alloc]init];
    ...
    NSString *sqlsx = [NSString stringWithFormat:@"select * from %@ where newMsgs = 1", @"newFriend"];
      
    FMResultSet *rs=[db executeQuery:sqlsx];
    while ([rs next]) {
        WH_JXFriendObject *user=[[WH_JXFriendObject alloc] init];
        [super userFromDataset:user rs:rs];
