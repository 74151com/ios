...
//    dic[@"sign"] = @"602DB4C8DF0203B34DBB69B3A1E1F0AC";
    ...
    if (type == GKNetworkingTypeGet) {
        [manager GET:url parameters:dic headers:@{} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            !success ? : success(responseObject);
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            !failure ? : failure(error);
        }];
    }else {
        [manager POST:url parameters:dic headers:@{} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            !success ? : success(responseObject);
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
