...
@property (nonatomic, weak) WH_NewGroupView *groupView;
...
//    }
    ...
    if ([g_config.isOpenPositionService intValue] == 0) {
        ...
        sels = [NSMutableArray arrayWithArray:@[@"onNewRoomCreate", @"onSearch", @"showScanViewController", @"onNear",@"onReceiving",@"searchPublicNumber"]];
    }else {
        titles = [NSMutableArray arrayWithArray:@[Localized(@"JX_LaunchGroupChat"), Localized(@"JX_AddFriends"), Localized(@"JX_Scan"), Localized(@"JX_Receiving"),Localized(@"JX_SearchPublicNumber")]];
        images = [NSMutableArray arrayWithArray:@[@"icon_group_chat_entry",  @"icon_add_friend", @"icon_scan", @"icon_payment_received",@"message_near_receiving",@"message_search_publicNumber"]];
        sels = [NSMutableArray arrayWithArray:@[@"onNewRoomCreate", @"onSearch", @"showScanViewController",@"onReceiving",@"searchPublicNumber"]];
    }
    ...
    if ([g_config.isCommonCreateGroup intValue] == 1 && g_myself.role.count <= 0) {
        ...
        [sels removeObject:@"onNewRoomCreate"];
    }
    if ([g_config.isOpenPositionService intValue] == 1) {
        ...
        [sels removeObject:@"onNear"];
         ...
         [titles removeObject:Localized(@"WaHu_JXNear_WaHuVC_NearPer")];
         [images removeObject:@"mefssage_near_receiving"];
         [sels removeObject:@"onNear"];
    
        [titles removeObject:Localized(@"JX_Receiving")];
        [images removeObject:@"icon_payment_received"];
...
-(void)xmppRoomDidCreate:(XMPPRoom *)sender{
    [g_server addRoom:_createRoom userArray:@[] isPublic:YES isNeedVerify:NO category:0 toView:self];
...
-(void)onNewRoom{
    ...
    sendView.title = _chatRoom.roomTitle;
    sendView.roomJid = _chatRoom.roomJid;
    sendView.roomId = _createRoom.roomId;
    sendView.chatRoom = _chatRoom;
    sendView.room = _createRoom;
  
    WH_JXUserObject * user = [[WH_JXUserObject alloc]init];
    user = [user getUserById:_chatRoom.roomJid];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([dict.user.msgsNew intValue]==0){
        cell.readImageView.hidden= YES;// = [UIImage imageNamed:@"yidu"];
    }else{
        cell.readImageView.hidden= YES;// = [UIImage imageNamed:@"weidu"];
    }
    //隐藏或显示快速回复按钮
    cell.isMsgVCCome = NO;
...
//        cell.lbSubTitle.attributedText = attr;
    ...
    if ([dict.message.type intValue] == kWCMessageTypeText || flag) {
        ...
        if ([lastContet containsString:@"撤回了一条消息"]) {
...
//            p.userId = [@"10000" longLongValue];
//            p.userNickName = @"客服共众号";
//            p.role = [NSNumber numberWithInt:3];
//            [_createRoom.members addObject:p];
//            [self actionQuit];
    ...
    if( [aDownload.action isEqualToString:wh_act_roomAdd] ){
        _createRoom.roomId = [dict objectForKey:@"id"];
        ...
        [g_server WH_addRoomMemberWithRoomId:_createRoom.roomId userArray:@[] toView:self];//用接口即可
        [self onNewRoom];
...
#pragma mark 进群验证
    ...
    if( [aDownload.action isEqualToString:wh_act_UserGet] ){
        ...
        if(_isclickMemberPrivateChat==YES){
            _isclickMemberPrivateChat = NO;
            int numberStr = 100;
            ...
            WH_NewGroupView *groupView = [[WH_NewGroupView alloc]initWithFrame:[UIScreen mainScreen].bounds];
            groupView.numberStr = numberStr;
            groupView.groupblcok = ^(NSString *nameStr, NSString *numStr) {
                
                NSString* s = [XMPPStream generateUUID];
                s = [[s stringByReplacingOccurrencesOfString:@"-" withString:@""] lowercaseString];
                    
                _createRoom.roomJid= s;
                _createRoom.name   = nameStr;
                _createRoom.maxCount = [numStr intValue];
                _createRoom.desc   = nameStr;
                _createRoom.userId = [g_myself.userId longLongValue];
                _createRoom.userNickName = MY_USER_NAME;
                _createRoom.showRead = NO;
                _createRoom.showMember = YES;
                _createRoom.allowSendCard = YES;
                _createRoom.isLook = YES;
                _createRoom.isNeedVerify = NO;
                _createRoom.allowInviteFriend = YES;
                _createRoom.allowUploadFile = YES;
                _createRoom.allowConference = YES;
                _createRoom.allowSpeakCourse = YES;
                
                _chatRoom = [[JXXMPP sharedInstance].roomPool createRoom:s title:nameStr];
                _chatRoom.delegate = self;
                
                [_wait start:Localized(@"JXAlert_CreatRoomIng") delay:30];
                
                
                NSLog(@"nameStr = %@ %@",nameStr,numStr);
            };
            [[UIApplication sharedApplication].keyWindow addSubview:groupView];
            groupView.autoresizingMask = UIViewAutoresizingNone;
...
-(void)insertRoom{
    WH_JXUserObject* user = [[WH_JXUserObject alloc]init];
    user.userNickname = _createRoom.name;
    user.userId = _createRoom.roomJid;
    user.userDescription = _createRoom.desc;
    user.roomId = _createRoom.roomId;
    user.content = Localized(@"JX_WelcomeGroupChat");
    user.showRead =  [NSNumber numberWithBool:_createRoom.showRead];
    user.showMember = [NSNumber numberWithBool:_createRoom.showMember];
    user.allowSendCard = [NSNumber numberWithBool:_createRoom.allowSendCard];
    user.chatRecordTimeOut = _createRoom.chatRecordTimeOut;
    user.talkTime = [NSNumber numberWithLong:_createRoom.talkTime];
    user.allowInviteFriend = [NSNumber numberWithBool:_createRoom.allowInviteFriend];
    user.allowUploadFile = [NSNumber numberWithBool:_createRoom.allowUploadFile];
    user.allowConference = [NSNumber numberWithBool:_createRoom.allowConference];
    user.allowSpeakCourse = [NSNumber numberWithBool:_createRoom.allowSpeakCourse];
    [user insertRoom];
}
-(void)showChatView{
    [_wait stop];
