...
#import "WH_JXMessage_EditCell.h"
...
@property (nonatomic, assign) NSInteger withEditIndex;
@property (nonatomic, copy) NSString *withEditContent;
...
- (id)init
{
    ...
    [g_notify addObserver:self selector:@selector(NoteBookCellClick:) name:kCellShowNoteBookNotifaction object:nil];
...
//跳转到笔记页面
-(void)NoteBookCellClick:(NSNotification *) notification{
    if (recording) {
        return;
    }
    WH_JXMessageObject *msg = notification.object;
    
    NSArray *videoUrlsArr  = [msg.fileName componentsSeparatedByString:@"===="];
    NSArray *fileNameNoteId = [videoUrlsArr lastObject];
    NSArray *contentNoteStr =  [msg.content componentsSeparatedByString:@"===="];
  //  content    __NSCFString *    "Anh đang đi làm không đi chơi |Chị ơi sao mà em nói chuyện anh my name"
    TMineNoteBookDetialListTVc *vc = [TMineNoteBookDetialListTVc alloc];
   // TMineNoteBookDetialVc *vc = [TMineNoteBookDetialVc alloc];
    vc.dictArr = [videoUrlsArr.firstObject mj_JSONObject];
    vc.titleStr = [contentNoteStr firstObject];
    vc.contentStr = [contentNoteStr lastObject];
    vc.msg = msg;
    vc = [vc init];
...
- (void)messageSync
{
    ...
    if ([self.chatPerson.downloadTime timeIntervalSince1970] < [self.chatPerson.timeSend timeIntervalSince1970] && _taskList.count<=0) {
        ...
        if (syncTimeLen != -2) {
            ...
            if([self.roomJid length]>0)
            {
                [g_server WH_tigaseMucMsgsWithRoomId:s StartTime:0 EndTime:0 PageIndex:0 PageSize:PAGECOUNT toView:self];
            }else{
...
- (void) customView {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        _moreView.onNoteBook = @selector(onNoteBook);
...
-(void)initAudio{
    ...
    [g_notify addObserver:self selector:@selector(deleteMsgCome:) name:@"kXMPPDeleteMsg_WHNotifaction" object:nil];
    ...
    // 撤回编辑
    [g_notify addObserver:self selector:@selector(withEditNotifi:) name:kXMPPMessageWithEdit_WHNotification object:nil];
...
-(void)unInitAudio{
    ...
    [g_notify removeObserver:self name:kXMPPMessageWithEdit_WHNotification object:nil];
...
- (void)sendIt:(id)sender {
    ...
    if (message.length > 0) {
        ...
        NSString *objstr =[self encryptAES:message];
...
//key
static NSString *const PSW_AES_KEY = @"1234567876666666";
//偏移量
NSString *const kInitVector = @"1112222211111121";
//密钥长度
size_t const kKeySize = kCCKeySizeAES128; //kCCKeySizeAES256;//
    ...
    if (msg) {
        switch ([msg.type intValue]) {
            case kWCMessageTypeText:
                ...
                break;
            case kWCMessageTypeWithEdit:
                return [WH_JXMessage_EditCell getChatCellHeight:msg];
                break;
            ...
            case kWCMessageTypeShake:
                ...
                break;
            case kWCMessageTypeNoteBook:
                return [WH_JXNoteBook_WHCell getChatCellHeight:msg];
                break;
...
- (WH_JXBaseChat_WHCell *)getCell:(WH_JXMessageObject *)msg indexPath:(NSIndexPath *)indexPath{
    ...
    switch ([msg.type intValue]) {
        case kWCMessageTypeText:
            ...
            break;
        case kWCMessageTypeWithEdit:
            cell = [self WH_creat_WHEditMessageCell:msg indexPath:indexPath];
            break;
        case kWCMessageTypeImage:
        ...
        case kWCMessageTypeTransfer:
            ...
            break;
        case kWCMessageTypeNoteBook:
            cell = [self WH_creat_WHNoteBookCell:msg indexPath:indexPath];
            break;
        case kWCMessageTypeSystemImage1:
        ...
        case kWCMessageTypeShake:
            ...
            break;
        case kWCMessageTypeMergeRelay: //合并转发
...
//文本
- (WH_JXBaseChat_WHCell *)WH_creat_WHEditMessageCell:(WH_JXMessageObject *)msg indexPath:(NSIndexPath *)indexPath{
    NSString * identifier = @"WH_JXMessage_EditCell";
    if ([msg.isReadDel boolValue]) {
        identifier = [NSString stringWithFormat:@"WH_JXMessage_EditCell_%ld",indexPath.row];
    }
    WH_JXMessage_EditCell *cell=[_table dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[WH_JXMessage_EditCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
//笔记
- (WH_JXNoteBook_WHCell *)WH_creat_WHNoteBookCell:(WH_JXMessageObject *)msg indexPath:(NSIndexPath *)indexPath {
    NSString * identifier = @"WH_JXNoteBook_WHCell";
    WH_JXNoteBook_WHCell *cell=[_table dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[WH_JXNoteBook_WHCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    return cell;
}
...
#pragma mark  移除当前的群用户的聊天记录
-(void)deleteMsgCome:(NSNotification *)notifacation{
    
    NSDictionary *msg = (NSDictionary *)notifacation.object;
    
   [_lastMsg deleteWithFromUser:msg[@"toUserId"] roomId:self.roomJid];
    for(NSInteger i=[_array count]-1;i>=0;i--){
        WH_JXMessageObject *p=[_array objectAtIndex:i];
        if([p.fromUserId isEqualToString:[msg objectForKey:@"toUserId"]]){//如果找到被撤回的那条消息
...
-(void)newMsgCome:(NSNotification *)notifacation{
    ...
    if (self.roomJid || msg.isGroup) {//是房间
        ...
        if ([msg.fromUserId isEqualToString:MY_USER_ID] && [msg.type intValue] == kWCMessageTypeWithEdit) {
            ...
            newMsg.isNotUpdateHeight = NO;
            newMsg.fileName = @"950";
            newMsg.isShowTime = NO;
            newMsg.type = [NSNumber numberWithInt:kWCMessageTypeWithEdit];
...
-(void)newReceipt:(NSNotification *)notifacation{//新回执
    ...
    if ([msg.type intValue] == kWCMessageTypeWithEdit) {
        WH_JXMessageObject *msg1 = _array[self.withEditIndex];
        msg1.isShowTime = NO;
        msg1.type = [NSNumber numberWithInt:kWCMessageTypeWithEdit];
        msg1.messageId = msg.messageId;
        msg1.isNotUpdateHeight = NO;
        msg.fileName = @"950";
        msg1.content = msg.content;
        NSString* s;
        if([self.roomJid length]>0)
            s = self.roomJid;
        else
        ...
        msg1.fromUserId = MY_USER_ID;
        msg1.toUserId = s;
        if (self.withEditIndex == _array.count - 1) {
            self.lastMsg.content = msg1.content;
        }
        [msg1 updateLastSend:UpdateLastSendType_None];
        [msg1 update];
        [msg1 notifyNewMsg];
        [_wait stop];
        [_table WH_reloadRow:(int)self.withEditIndex section:0];
        return;
    }
    if ([msg.type intValue] == kWCMessageTypeNoteBook) {
//        WH_JXMessageObject *msg1 = _array[self.withEditIndex];
//        msg1.isShowTime = NO;
//        msg1.type = [NSNumber numberWithInt:kWCMessageTypeNoteBook];
//        msg1.messageId = msg.messageId;
//        msg1.isNotUpdateHeight = NO;
//        msg.fileName = msg1.fileName;
//        msg1.content = msg.content;
//        NSString* s;
//        if([self.roomJid length]>0)
//            s = self.roomJid;
...
- (void)onNoteBook {
    ...
    TMineNoteBookListVc *collectVC = [[TMineNoteBookListVc alloc] init];
    collectVC.delegate = self;
    collectVC.isSend = YES;
    collectVC.isMine = YES;
...
- (void) noteBookVc:(TMineNoteBookListVc *)weiboVC didSelectWithData:(NSDictionary *)data{
    ...
    NSString * text111 = [NSString stringWithFormat:@"%@",data[@"body"][@"title"]];
    NSString *textStr =  [NSString stringWithFormat:@"%@",data[@"body"][@"text"]];
    
    NSArray *videoUrlsArr = data[@"body"][@"images"];
    
    msg.content      = [NSString stringWithFormat:@"%@====%@",text111,textStr];
    msg.fileName     =  [NSString stringWithFormat:@"%@====%@",[videoUrlsArr mj_JSONString],data[@"noteId"]];
    msg.type         = [NSNumber numberWithInt:kWCMessageTypeNoteBook];
    msg.isSend       = [NSNumber numberWithInt:transfer_status_ing];
    msg.isRead       = [NSNumber numberWithBool:NO];
    msg.isReadDel    = self.chatPerson.isOpenReadDel;
    ...
    if (self.isGroupMessages) {
        self.groupMessagesIndex ++;
        if (self.groupMessagesIndex < self.userIds.count) {
            [self noteBookVc:weiboVC didSelectWithData:data];
        }else if (self.userIds){
            self.groupMessagesIndex = 0;
            [g_App showAlert:Localized(@"JX_SendComplete")];
            return;
...
- (void)selectFavoritWithString:(NSString *)str {
    ...
    if ([file length]>0) {
        ...
        msg.fileName     = file;
        if ([str containsString:@"emoj_dice_"]||[str containsString:@"shaizi2"]) {
            
            int result1 = arc4random()%6+1;  //产生1～6的数
            
            NSString *fileStr = [NSString stringWithFormat:@"emoj_dice_%d.gif",result1];
            msg.content    = fileStr;
            msg.type         = [NSNumber numberWithInt:kWCMessageTypeGif];
        }else if ([str containsString:@"emoj_cquan_"]||[str containsString:@"jsbcaiquan"]) {
            
            int result1 = arc4random()%3+1;  //产生1～6的数
            
            NSString *fileStr = [NSString stringWithFormat:@"emoj_cquan_%d.gif",result1];
            msg.content    = fileStr;
            msg.type         = [NSNumber numberWithInt:kWCMessageTypeGif];
        }else{
...
// 发送收藏表情
- (void)selectFavoritWithStringbbb:(NSString *)str {
...
//                if ([msg.type intValue] == kWCMessageTypeNoteBook) {
//                    msg.type = [NSNumber numberWithInt:kWCMessageTypeText];
//                    msg.content = [NSString stringWithFormat:@"[%@%@]", @"笔记" ,Localized(@"WaHu_JXMain_WaHuViewController_Message")];
//                }
                
//                if ([msg.type intValue] == kWCMessageTypeMergeRelay) {
//                    msg.type = [NSNumber numberWithInt:kWCMessageTypeText];
//                    msg.content = [NSString stringWithFormat:@"[%@%@]", @"合并转发" ,Localized(@"WaHu_JXMain_WaHuViewController_Message")];
//                }
    ...
    }else if ([aDownload.action isEqualToString:wh_act_tigaseEditMsg]) { //编辑消息
        
        if (self.withEditIndex >= 0) {
            [_wait start];
            WH_JXMessageObject *msg = _array[self.withEditIndex];
            ...
            newMsg.isNotUpdateHeight = NO;
            newMsg.messageId    = msg.messageId;
            newMsg.content      = _withEditContent;//@"哈哈哈哈哈";// msg.content;
            newMsg.type         = [NSNumber numberWithInt:kWCMessageTypeWithEdit];
...
- (void)onTransfer {
    ...
    if(_roomJid.length>0){
        ...
        vc.didSelect = @selector(onAfterSelectTransfer:);
        vc = [vc init];
        [g_navigation pushViewController:vc animated:YES];
    }else{
...
-(void)onAfterSelectTransfer:(WH_JXSelectFriends_WHVC*)vc{
    WH_JXUserObject * member=(WH_JXUserObject*)vc;
//    for(NSNumber* n in vc.set){
//        memberData *user;
//        if (vc.seekTextField.text.length > 0) {
//            user = vc.searchArray[[n intValue] % 1000];
...
//       /// [g_meeting sendMeetingInvite:s toUserName:user.userName roomJid:self.roomJid callId:self.meetingNo type:type];
//    }
   // memberData * member=vc.set;
    
    WH_JXTransfer_WHViewController *transferVC = [WH_JXTransfer_WHViewController alloc];
    transferVC.wh_user = member;
    transferVC.delegate = self;
    transferVC.roomId = _roomJid;
    transferVC = [transferVC init];
    [g_navigation pushViewController:transferVC animated:YES];
...
#pragma mark - 笔记delegate
- (void)NoteBookToUser:(NSDictionary *)dict {
    [self hideKeyboard:NO];
    if ([dict[@"id"] length]>0) {
        WH_JXMessageObject *msg=[[WH_JXMessageObject alloc] init];
        msg.timeSend     = [NSDate date];
        ...
        if([self.roomJid length]>0){
            ...
            msg.isGroup = YES;
            msg.fromUserName = _userNickName;
           // [4]    (null)    "toUserId" : 10000025
            memberData *roleData = [_room getMember:touserId];
            
            msg.toUserName = roleData.userName.length>0?roleData.userName:roleData.userNickName;
        }
        else{
            msg.toUserId     = chatPerson.userId;
            msg.isGroup = NO;
...
#pragma mark 长按编辑
- (void)chatCell:(WH_JXBaseChat_WHCell *)chatCell withEditIndexNum:(int)indexNum {
    if ([self isNoAllowedOpearitionOnJinYanStatus]) {
        //禁言情况下
        [GKMessageTool showText:@"禁言状态下不能进行此操作！"];
        return;
    ...
    WH_JXMessageObject *msg = _array[indexNum];
    self.withEditIndex = indexNum;
    int type = 1;
    if (self.roomJid) {
        type = 2;
    }
    
    AAShen_RoomInputPwdView *vcshow = [AAShen_RoomInputPwdView XIBAAShen_RoomInputPwdView];
    vcshow.frame =  CGRectMake(0, 0, JX_SCREEN_WIDTH, JX_SCREEN_HEIGHT);;
    [[UIApplication sharedApplication].keyWindow addSubview:vcshow];
    
    vcshow.blockpw = ^(NSString * _Nonnull pwd) {
        
        if (pwd.length>0) {
            _withEditContent = pwd;
            [g_server WH_tigaseEditMsgWithMsgId:msg.messageId type:type deleteType:2 roomJid:self.roomJid msgContent:pwd toView:self];
        }
    
        [vcshow removeFromSuperview];
...
- (void)chatCell:(WH_JXBaseChat_WHCell *)chatCell favoritIndexNum:(int)indexNum type:(CollectType)collectType{
    ...
    if([type intValue]== 8){
        ...
        [dataDict setValue:msg.fileName forKey:@"msg"];
        ...
        /*
         NSArray *imageVideos = [msg.fileName mj_JSONObject];
        NSMutableString *stringM = [NSMutableString string];
        for (NSDictionary *dictUrl in imageVideos) {
            
           [stringM appendString:[NSString stringWithFormat:@"%@,",[dictUrl objectForKey:@"oUrl"]]];
            
         }
        NSString *tempStr = stringM.copy;
        if ([tempStr length] > 0) {
        ...
        [dataDict setValue:msg.content forKey:@"collectContent"];
        [dataDict setValue:tempStr forKey:@"msg"];
        [dataDict setValue:@8 forKey:@"type"];
        [dataDict setValue:msg.messageId forKey:@"collectMsgId"];
...
//    [g_server userEmojiAddWithUrl:msg.content toView:self];
    ...
    /**
     http://82.157.30.116:8092/user/emoji/add?&secret=0fa08829e028339641f4d648bfcd6b62&emoji=[{"collectContent":"今天下午吃早餐吃药没有人","msg":"http:\/\/82.157.30.116:8089\/u\/8\/10000008\/202402\/o\/38651f84416c471ca5205f7cce75fea3.jpg,http:\/\/82.157.30.116:8089\/u\/8\/10000008\/202402\/o\/071e66efb6b64ead9ce16f127fe679f0.jpg","type":1,"collectMsgId":"65ccdba2061c572f4cc94912","collectType":1}]&time=1707924391&access_token=bc26d7dc2ae94950ba1c87020fdf9e1d
...
//            NSMutableString *types = [NSMutableString string];
            ...
            for (NSInteger i = 0; i < self.selectMoreArr.count; i ++) {
                ...
                if ([msg.type intValue] == kWCMessageTypeText || [msg.type intValue] == kWCMessageTypeImage || [msg.type intValue] == kWCMessageTypeVoice || [msg.type intValue] == kWCMessageTypeVideo || [msg.type intValue] == kWCMessageTypeFile) {
                    ...
                    }else if ([msg.type intValue] == kWCMessageTypeNoteBook) {
                        collectType = CollectTypeNoteBook;
                    }else {
...
//                    }
                    ...
                    if([type intValue]== 8){
                        ...
                        [dataDict setValue:msg.fileName forKey:@"msg"];
                        [dataDict setValue:type forKey:@"type"];
                        [dataDict setValue:@2 forKey:@"collectType"];
                    }else{
                        [dataDict setValue:msg.messageId forKey:@"msgId"];
                        [dataDict setValue:msg.content forKey:@"msg"];
                        [dataDict setValue:type forKey:@"type"];
...
#pragma mark ---  消息编辑
- (void)withEditNotifi:(NSNotification *) notif {
    WH_JXMessageObject *msg = notif.object;
