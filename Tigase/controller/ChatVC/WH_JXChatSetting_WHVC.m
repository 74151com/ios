...
//    h+=iv.frame.size.height + marginHei;
    ...
    iv = [self MiXin_createMiXinButton:Localized(@"JX_BlackList") superView:syzxView orginY:0 drawTop:NO drawBottom:YES icon:nil click:nil];
    _chatBlackView = iv;
    [self MiXin_create_MiXinSwitch:iv defaule:[self.user.status intValue]==1?YES:NO click:@selector(blackChatAction:)];
...
//    h+=iv.frame.size.height;
    ...
    iv = [self MiXin_createMiXinButton:Localized(@"JX_DelMsgTwoSides") superView:syzxView orginY:iv.bottom drawTop:NO drawBottom:YES icon:nil click:@selector(cleanTwoSidesMessageLog)];
...
- (void)setInsertH:(CGFloat)insertH{
    ...
    insertView.top = insertView.top + insertH-30;
...
// 黑名单
- (void)blackChatAction:(UISwitch *)switchView {
    if (switchView.isOn) {
        
       [g_server WH_addBlacklistWithToUserId:self.user.userId toView:self];
    }else {
        [g_server WH_delBlacklistWithToUserId:self.user.userId toView:self];
        
    }
     
    
//    if(self.wh_friendStatus == friend_status_friend){
//        if(self.wh_friendStatus == friend_status_black)  [self.wh_user doSendMsg:XMPP_TYPE_BLACK content:nil];
//            //取消黑名单
...
-(void)WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if( [aDownload.action isEqualToString:wh_act_UserGet] ){
        ...
        if(_isclickMessageLog==YES){
            _isclickMessageLog = NO;
...
//        [g_navigation pushViewController:vc animated:YES];
//    }
    if ([aDownload.action isEqualToString:wh_act_BlacklistDel]) {//删除好友
