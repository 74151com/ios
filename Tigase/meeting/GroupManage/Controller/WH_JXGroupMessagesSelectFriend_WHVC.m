...
@interface WH_JXGroupMessagesSelectFriend_WHVC () <UITextFieldDelegate, WH_JXSelectLabels_WHVCDelegate,JXSelectGroupSendVCDelegate>
...
//    [backView addSubview:cancelBtn];
    ...
    _selectGroupTip.text =@"选择群组群发";// Localized(@"JX_SelectGroupGroupSending");
...
- (void)nextBtnAction:(UIButton *)btn {
    ...
    for (NSInteger i = 0; i < self.selGroupsArr.count; i++) {
        WH_JXUserObject *group = _selGroupsArr[i];
    ...
    for (NSInteger i = 0; i < self.selMailistArr.count; i++) {
        WH_JXUserObject *user = self.selMailistArr[i];
