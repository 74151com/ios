    ...
    if( [aDownload.action isEqualToString:wh_act_UserGet] ){
        ...
        if(_isclickMemberPrivateChat==YES){
            ...
            if([[dict objectForKey:@"vip"] intValue]>0){
                memberData *roleData = [wh_room getMember:g_myself.userId];
                if ([roleData.role intValue] == 1||[roleData.role intValue] == 2) {//群主
                    CGFloat viewH = 226;
                    if (THE_DEVICE_HAVE_HEAD) {
                        viewH = 226+24;
                    }
                    WH_CustomActionSheetView *share = [[WH_CustomActionSheetView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, viewH) WithTitle:@"清空全部群成员聊天记录，执行后永久删除所有群成员设备上的聊天记录，并不可恢复。" sureBtnColor:HEXCOLOR(0x0093FF)];
                    ...
                    share.wh_okActionBlock = ^{
                        [weakShare hideView];
                       [g_server wh_twoWayDelectRoomMsg:g_myself.userId?:@"" roomId:weakSelf.wh_room.roomJid?:@"" toView:weakSelf];
                    };
                    share.wh_cancelActionBlock = ^{
                        [weakShare hideView];
                    };
                }else{
                    [GKMessageTool showText:@"您不是该群群主/管理员，不能进行此操作！"];
                    return;
                }
...
//        [p release];
    ...
    p.backgroundColor = [UIColor clearColor];
    if([title isEqualToString:@"💎 清空群所有成员聊天记录"]){
        p.textColor = [UIColor redColor];
    }else{
...
//    }
    ...
    ///清空群所有成员聊天记录
    self.wh_iv = [self WH_createMiXinButton:@"💎 清空群所有成员聊天记录" drawTop:NO drawBottom:YES must:NO click:@selector(cleanAllMessageLog) ParentView:topFourView];
    self.wh_iv.frame = CGRectMake(0, fourViewHei, topFourView.frame.size.width, HEIGHT);
    membHeight+=self.wh_iv.frame.size.height;
    fourViewHei += self.wh_iv.frame.size.height;
