...
//    membHeight = CGRectGetMaxY(label.frame) + 12;
    ...
    // 允许普通成员私聊 Localized(@"JX_AllowMemberChat")
    UIView *xyslView = [self createBGViewWithOrginY:membHeight height:HEIGHT supView:self.cView];
    iv = [self WH_createMiXinButton:[NSString stringWithFormat:@"💎 %@",@"禁止群成员互相添加好友"] supView:xyslView drawTop:NO drawBottom:NO must:NO click:nil];
...
- (void)allowSendCardSwitchAction:(UISwitch *)allowSendCardSwitch {
    ...
    _allowSendCardSwitch = allowSendCardSwitch;
    
    [g_server getUser:MY_USER_ID toView:self];
    
   /*
    memberData *data = [self.room getMember:g_myself.userId];
    if ([data.role intValue] != 1||[data.role intValue] != 2) { //群主才有权限操作
        [g_App showAlert:Localized(@"WaHu_JXRoomMember_WaHuVC_NotGroupMarsterCannotDoThis")];
    ...
    if( [aDownload.action isEqualToString:wh_act_UserGet] ){
        ...
        if(_isclickMemberPrivateChat==YES){
            ...
            if([[dict objectForKey:@"vip"] intValue]>0){
                ...
                self.room.allowSendCard = _allowSendCardSwitch.on;
            }else{
                [_allowSendCardSwitch setOn:!_allowSendCardSwitch.isOn];
