...
@property (strong, nonatomic) UILabel *labelTimeStatus;
...
- (void)initializeAgoraEngine{
    ...
    config.appId = _model.appId;
    config.channelProfile = AgoraChannelProfileLiveBroadcasting;
    
    self.agorakit = [AgoraRtcEngineKit sharedEngineWithConfig:config delegate:self];
    
    NSString *channelName = _model.channel;
    ...
    videoCanvas.uid =[MY_USER_ID integerValue] ;
    ...
    int result = [self.agorakit joinChannelByToken:_model.ownToken channelId:channelName uid:[MY_USER_ID integerValue]  mediaOptions:options joinSuccess:^(NSString * _Nonnull channel, NSUInteger uid, NSInteger elapsed) {
        
        [self setLocalVideo];
       
        NSLog(@"join uid %@",channel);
        NSLog(@"join channel %lu",(unsigned long)uid);
        NSLog(@"join elapsed %lu",(unsigned long)elapsed);
...
- (void)setupVideo{
    ...
    AgoraVideoEncoderConfiguration *config = [[AgoraVideoEncoderConfiguration alloc]initWithSize:AgoraVideoDimension640x360 frameRate:AgoraVideoFrameRateFps15 bitrate:AgoraVideoBitrateStandard orientationMode:AgoraVideoOutputOrientationModeAdaptative mirrorMode:AgoraVideoMirrorModeAuto];
    //CGSizeMake(self.view.frame.size.width, self.view.frame.size.height)
  //  AgoraVideoEncoderConfiguration *configuration = [[AgoraVideoEncoderConfiguration alloc]initWithSize:CGSizeZero frameRate:AgoraVideoFrameRateFps15 bitrate:1.0 orientationMode:AgoraVideoOutputOrientationModeAdaptative mirrorMode:AgoraVideoMirrorModeAuto];
...
-(void)rtcEngine:(AgoraRtcEngineKit *)engine firstRemoteVideoFrameOfUid:(NSUInteger)uid size:(CGSize)size elapsed:(NSInteger)elapsed{
    ...
    NSLog(@"firstRemoteVideoFrameOfUid:Uid %d",uid);
...
- (void)rtcEngine:(AgoraRtcEngineKit * _Nonnull)engine
 didRejoinChannel:(NSString * _Nonnull)channel withUid:(NSUInteger)uid elapsed:(NSInteger) elapsed{
    ...
    NSLog(@"成功重新加入频道回调。:Uid %d",uid);
}
// self joined success 成功加入频道回调。
- (void)rtcEngine:(AgoraRtcEngineKit *)engine didJoinChannel:(NSString*)channel withUid:(NSUInteger)uid elapsed:(NSInteger) elapsed {

    NSLog(@"didJoinChannel:Uid %d",uid);
}
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine didJoinedOfUid:(NSUInteger)uid elapsed:(NSInteger)elapsed {
    NSLog(@"didJoinedOfUid:Uid %d",uid);
    NSLog(@"Uid");
...
- (void)rtcEngine:(AgoraRtcEngineKit * _Nonnull)engine localAudioStats:(AgoraRtcLocalAudioStats * _Nonnull)stats NS_SWIFT_NAME(rtcEngine(_:localAudioStats:)){
    ...
    NSLog(@"tokenPrivilegeWillExpire %@",stats);
}
...
- (void)rtcEngine:(AgoraRtcEngineKit * _Nonnull)engine didLocalVideoTranscoderErrorWithStream:(AgoraTranscodingVideoStream * _Nonnull )stream
    ...
    NSLog(@"tokenPrivilegeWillExpire %@",errorCode);
...
tokenPrivilegeWillExpire:(NSString *_Nonnull)token{
    ...
    NSLog(@"tokenPrivilegeWillExpire %@",token);
    
}
- (void)rtcEngineConnectionDidInterrupted:(AgoraRtcEngineKit *)engine {
     NSLog(@"Interrupt");
    NSLog(@"rtcEngineConnectionDidInterrupted= %@",engine.getCallId);
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine remoteAudioStateChangedOfUid:(NSUInteger)uid state:(AgoraAudioRemoteState)state reason:(AgoraAudioRemoteReason)reason elapsed:(NSInteger)elapsed{
      ...
      NSLog(@"remoteAudioStateChangedOfUid %zd 声音:%zd",uid,state);
...
- (void)rtcEngine:(AgoraRtcEngineKit *)engine didOccurError:(AgoraErrorCode)errorCode {
    ...
    NSLog(@"Error %ld %@",(long)errorCode,engine);
...
- (void) customView {
    ...
    if (_isAudio) {
        ...
        [g_server WH_getHeadImageLargeWithUserId:self.toUserId userName:self.toUserName imageView:_headerImage];
         //getHeadImageLarge:self.toUserId imageView:_headerImage ];
        ...
        //显示时间
        _labelTimeStatus = [[UILabel alloc] init];
        _labelTimeStatus.frame = CGRectMake((JX_SCREEN_WIDTH-80)/2, JX_SCREEN_HEIGHT-220 + 10, 80, 30);
        _labelTimeStatus.textColor = [UIColor whiteColor];
        //_labelTimeStatus.backgroundColor = [UIColor grayColor];
        _labelTimeStatus.font = [UIFont systemFontOfSize:14];
        _labelTimeStatus.textAlignment = NSTextAlignmentCenter;
        _labelTimeStatus.text = @"00:00";
        [self.view addSubview:_labelTimeStatus];
