...
#import "JXAVCallGroupVideoViewController.h"
...
@interface JXAVCallGroupVideoViewController ()<AgoraRtcEngineDelegate,UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation JXAVCallGroupVideoViewController
...
//        [localVideoMutedIndicator addSubview:localVideo];
        ...
        self.showTimeLabel.frame = CGRectMake(0, 44, JX_SCREEN_WIDTH, 20);
...
//        for (int i=0; i<self.talkArray.count; i++) {
        ...
        model.type = @"1";
        model.userId = [NSString stringWithFormat:@"%zd",uid];
...
//        }
    ...
    NSLog(@"成功重新加入频道回调1: 自己的UserId%@  %zd",MY_USER_ID,uid);
}
 
//远端用户（通信场景）/主播（直播场景）加入当前频道回调。 新加入频道的远端用户/主播 ID
- (void)rtcEngine:(AgoraRtcEngineKit *)engine didJoinedOfUid:(NSUInteger)uid elapsed:(NSInteger)elapsed {
//    NSLog(@"远端用户（通信场景）/主播（直播场景）加入当前频道回调:Uid %d",uid);
    
    NSLog(@"成功重新加入频道回调2 新人: 自己的UserId%@  %zd",MY_USER_ID,uid);
...
- (void)panActionAdd:(UIButton *)pan {
   ...
   // vc.bbbbbb = YES;
    ...
    JXTalkModel *model = _talkArray.firstObject;
...
-(void)meetingAddMember:(WH_JXSelectFriends_WHVC*)vc{
    ...
    for(NSNumber* n in vc.set){
        ...
        NSString* s = [NSString stringWithFormat:@"%ld",user.userId];
         NSDictionary *dict= @{@"userId":s,@"userName":user.userName.length>0?user.userName:user.userNickName,@"roomJid":model.objectId.length>0?model.objectId:@"",@"callId":model.callId.length>0?model.callId:@"",@"type":@"120",@"roomId":model.roomId};
