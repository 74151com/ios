...
-(void)newMsgCome:(NSNotification *)notifacation{
    ...
    if([[WH_JXLiveJid_WHManager shareArray] contains:msg.toUserId] || [[WH_JXLiveJid_WHManager shareArray] contains:msg.fromUserId])
        return;
#endif
    if([msg.type intValue] == kWCMessageTypeAudioChatAccept){ //第三部
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
     ...
     self.model = [AgoraModel mj_objectWithKeyValues:dict];
    
    [g_default setObject:dict forKey:@"currentModel"];
    [g_default synchronize];
    //7d72365eb983485397e3e3f9d460bddb
    [g_meeting sendAsk:self.type toUserId:self.toUserId toUserName:self.toUserName meetUrl:_model.channel];
