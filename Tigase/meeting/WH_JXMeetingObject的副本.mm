...
-(void)sendAsk:(int)type toUserId:(NSString*)toUserId toUserName:(NSString*)toUserName meetUrl:(NSString *)meetUrl{
    ...
    [self doSendMsg:type content:content toUserId:toUserId toUserName:toUserId timeLen:0 fileName:meetUrl];
...
-(void)sendMeetingInvite:(NSString*)toUserId toUserName:(NSString*)toUserName roomJid:(NSString*)roomJid callId:(NSString*)callId callUser:(NSMutableArray*)callUser type:(int)type{
    ...
    msg.other = [callUser mj_JSONString];
...
//声网
-(void)doSendMsg:(int)type content:(NSString*)content toUserId:(NSString*)toUserId toUserName:(NSString*)toUserName timeLen:(int)timeLen fileName:(NSString *)fileName{
    WH_JXMessageObject *msg=[[WH_JXMessageObject alloc]init];
    msg.content      = content;
    msg.timeSend     = [NSDate date];
    msg.fromUserId   = MY_USER_ID;
...
-(void)newMsgCome:(NSNotification *)notifacation
{
    ...
    if ([msg.fromUserId isEqualToString:MY_USER_ID]) {
        if ([msg.type intValue] == kWCMessageTypeAudioChatAsk ||
            [msg.type intValue] == kWCMessageTypeVideoChatAsk ||
            [msg.type intValue] == kWCMessageTypeVideoMeetingInvite ||
            [msg.type intValue] == kWCMessageTypeAudioMeetingInvite ||
            [msg.type intValue] == kWCMessageTypeAudioChatAccept ||
            [msg.type intValue] == kWCMessageTypeVideoChatAccept) {
            return;
        }
    }
    ...
    if(msg==nil)
        return;
#ifdef Live_Version
//    if([[WH_JXLiveJid_WHManager shareArray] contains:msg.toUserId] || [[JXLiveJidManager shareArray] contains:msg.fromUserId])
      //  return;
#endif
    ...
    if([msg.toUserId isEqualToString:MY_USER_ID]){
        if([msg.type intValue] == kWCMessageTypeVideoChatAsk || [msg.type intValue] == kWCMessageTypeAudioChatAsk){
            if(g_meeting.isMeeting) return; //如果有别的通话 不弹出界面
...
-(void)sendReadyMsg:(WH_JXMessageObject*)msg{//发送准备好音视频通话的消息
    int n = [[NSDate date] timeIntervalSince1970]-[msg.timeSend timeIntervalSince1970];
    if(n>30)//如果时间差超过30秒，则放弃
        return;
    int k;
...
-(void)doAudioVideoMeeting:(acceptCallViewController*)vc{
    if(vc.isGroup){
        ...
        if(_msg.other.length>1){
             avVC.talkArray = [JXTalkModel mj_objectArrayWithKeyValuesArray:[_msg.other mj_JSONObject]];
        }
