...
#define wh_act_JPushSetRegId @"user/jgPush/setRegId"
...
#define wh_act_MsgAllMsglist @"b/circle/msg/list" //获取生活圈列表
//@"b/circle/msg/allMsglist" //获取所有生活圈列表
...
#define wh_act_codeyzfRecharge @"yzf/yzfRecharge"//易师兄支付
...
#define wh_act_transferRecordAdd @"user/transferRecord/add"//增加手动提现账号
...
#define wh_act_tigaseEditMsg @"tigase/changeContent" // 编辑消息
...
#define act_agorainfo @"agora/info" //声网

#define act_vipPaymentConfigList @"user/vipPaymentConfigList" //   int page，int limit

#define act_purchaseViinfo @"user/purchaseVip" //参数：vipConfigId

#define act_RealNameCertified_New @"user/realNameAuth" //新实名认证
#define act_RealNameCertified @"user/realNameCertified" //实名认证
#define act_UserGetDescribeVerifyToken  @"user/getDescribeVerifyToken"  // 获取实名认证token
#define act_UserCancelRealNameCertified @"user/cancelRealNameCertified" // 取消实名认证


//4.我的积分字段：个人信息接口添加字段:  score

//5.我的邀请列表：
#define act_UserSelectInviteUserList @"user/selectInviteUserList" //   参数：int pageIndex ，int pageSize

//6.兑换vip消耗积分数量 ：配置config 接口获取字段 ： exchangeVipScore

//7.兑换积分接口

#define act_UserexchangeVip @"user/exchangeVip"

//8.vip权益列表

#define act_UservipConfig @"user/vipConfig"
 
//10.新增笔记

#define act_NoteAddConfig @"note/add"//   参数同朋友圈，只是合并图片和视频同一数组，对象里面加type 2图片，4视频：images       例：{"ourl":xxx,"type":2}

//11.修改笔记

#define act_NoteUpdateConfig @"note/update" //   参数同上，多加参数  :  noteId

//12.笔记列表：

#define act_NoteListConfig @"note/getNoteList" //   参数：Integer pageIndex ，Integer pageSize
...
#define act_NoteDetailConfig @"note/detail" // 参数：String noteId
...
//-(void)WH_getact_NoteUpdateConfigUserId:(NSString*)noteId toView:(id)toView;
