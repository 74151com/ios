...
- (void) normalRequestData {
    ...
    if ([self.action isEqualToString:wh_act_Config]) {
        [self.httpManager GET:self.url parameters:nil headers:@{} progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        [self.httpManager POST:self.url parameters:self.params headers:@{} progress:^(NSProgress * _Nonnull downloadProgress) {
...
- (void) upLoadRequestData{
    ...
    //上传图片/文字，只能同POST
    [self.httpManager POST:self.url parameters:self.params headers:@{} constructingBodyWithBlock:^(id  _Nonnull formData) {
        // 上传文件
