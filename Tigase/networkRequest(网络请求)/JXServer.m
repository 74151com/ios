...
-(void)requestSuccess:(WH_JXConnection*)task
{
    ...
    @autoreleasepool {
        ...
        id resultObjectDict = [string mj_JSONObject];
        ...
        if(error){
            ...
            if ([task.action isEqualToString:act_h5Payment]) {
                ...
                if ([task.action isEqualToString:wh_act_deleteMemebers]) {
                    dict = resultObject;
                }if ([task.action isEqualToString:act_UserSelectInviteUserList]) {
                    dict = resultObject;
                }else{
...
-(void)WH_getHeadImageSmallWIthUserId:(NSString*)userId userName:(NSString *)userName imageView:(UIImageView*)iv{
    ...
    if([userId intValue]<10100 && [userId intValue]>=10000){
        ...
        iv.image = [UIImage imageNamed:[NSString stringWithFormat:@"im_notice_square"]];//@"im_10000"]];
        return;
    }
    // 支付公众号
...
-(void)doLoginOK:(NSDictionary*)dict user:(WH_JXUserObject*)user{
    ...
    if (user != nil) {
        ...
        myself.serInviteCode = user.serInviteCode;
    }
    ...
    [JPUSHService setAlias:myself.userId completion:^(NSInteger iResCode, NSString *iAlias, NSInteger seq) {
...
-(void)doSaveUser:(NSDictionary*)dict{
    ...
    if([dict objectForKey:@"serInviteCode"])
      myself.serInviteCode = [dict objectForKey:@"serInviteCode"];
...
-(void)saveDefaultSetting{
    ...
    [g_default setObject:myself.serInviteCode forKey:kMY_USER_SetINVITECODE];
...
//    myself.companyId =[g_default objectForKey:kMY_USER_COMPANY_ID];
    ...
    myself.serInviteCode =[g_default objectForKey:kMY_USER_SetINVITECODE];
...
//14.笔记详情
-(void)WH_getact_NoteDetailConfigUserId:(NSString*)noteId  toView:(id)toView{
    WH_JXConnection* p = [self addTask:act_NoteDetailConfig param:nil toView:toView];
    [p setPostValue:noteId forKey:@"noteId"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}

//13.删除笔记
#define act_NoteDeleteConfig @"/note/delete" //   参数：String noteId
-(void)WH_getact_NoteDeleteConfigUserId:(NSString*)noteId  toView:(id)toView{
    WH_JXConnection* p = [self addTask:act_NoteDeleteConfig param:nil toView:toView];
    [p setPostValue:noteId forKey:@"noteId"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}
...
//12.笔记列表：
-(void)WH_getact_NoteListConfigUserId:(NSString*)pageIndex toView:(id)toView{
    WH_JXConnection* p = [self addTask:act_NoteListConfig param:nil toView:toView];
    [p setPostValue:pageIndex forKey:@"pageIndex"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:@"15" forKey:@"pageSize"];
    [p go];
}

//11.修改笔记 #define act_NoteUpdateConfig @"/note/update" //   参数同上，多加参数  :  noteId
-(void)WH_getact_NoteUpdateConfigUserId:(NSString*)noteId imagesJson:(NSArray *)images  body:(NSString *)body title:(NSString *)title toView:(id)toView{
    WH_JXConnection* p = [self addTask:act_NoteUpdateConfig param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:body forKey:@"text"];
    [p setPostValue:[images mj_JSONString] forKey:@"images"];
    [p setPostValue:title forKey:@"title"];
    [p setPostValue:noteId forKey:@"noteId"];
    [p go];
}

//10.新增笔记
//参数同朋友圈，只是合并图片和视频同一数组，对象里面加type 2图片，4视频：images       例：{"ourl":xxx,"type":2}

-(void)WH_getact_NoteAddConfigUserId:(NSArray *)images  body:(NSString *)body title:(NSString *)title toView:(id)toView{
    WH_JXConnection* p = [self addTask:act_NoteAddConfig param:nil toView:toView];
...
- (void)WH_getPaySignWithPrice:(NSString *)price payType:(NSInteger)payType toView:(id)toView{
   ...
   // [p setPostValue:price forKey:@"price"]; //
    [p setPostValue:price forKey:@"vipConfigId"]; //
    /**
...
- (void)WH_transferToPeopleWithUserId:(NSString *)toUserId roomId:(NSString *)roomId money:(NSString *)money remark:(NSString *)remark time:(long)time secret:(NSString *)secret toView:(id)toView{
...
- (void)WH_wh_act_transferRecordAddParam:(NSDictionary *)param toView:(id)toView {
    WH_JXConnection *p = [self addTask:wh_act_transferRecordAdd param:nil toView:toView];
    [p setPostValue:self.access_token forKey:@"access_token"];
    [p setPostValue:[NSString stringWithFormat:@"%@", param[@"contextType"]] forKey:@"contextType"];
    [p setPostValue:[NSString stringWithFormat:@"%@", param[@"money"]] forKey:@"money"];
    [p setPostValue:[NSString stringWithFormat:@"%@", param[@"contextName"]] forKey:@"contextName"];
    [p setPostValue:[NSString stringWithFormat:@"%@", param[@"contextNo"]] forKey:@"contextNo"];
    if( [param[@"contextType"] intValue]==5){
        [p setPostValue:[NSString stringWithFormat:@"%@", param[@"contextBankName"]] forKey:@"contextBankName"];
    }
     
    [p go];
}
 
-(void)realNameCertifiedWithIDCard:(NSString*)iDCard withRealName:(NSString*)realName phone:(NSString *)phone frontImageUrl:(NSString*)frontUrl backImageUrl:(NSString *)backUrl toView:(id)toView{
    // /user/realNameAuth    name  idcard  idcardUrl  idcardBackUrl
    WH_JXConnection* p = [self addTask:act_RealNameCertified_New param:nil toView:toView];
    [p setPostValue:phone forKey:@"phone"];
    [p setPostValue:iDCard forKey:@"idcard"];
    [p setPostValue:realName forKey:@"name"];
    [p setPostValue:frontUrl forKey:@"idcardUrl"];
    [p setPostValue:backUrl forKey:@"idcardBackUrl"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}
...
// 实名认证
- (void)realNameCertifiedWithToView:(id)toView{
    
    WH_JXConnection* p = [self addTask:act_RealNameCertified param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}

// 获取实名认证token
- (void)userGetDescribeVerifyTokenToView:(id)toView {
    
    WH_JXConnection* p = [self addTask:act_UserGetDescribeVerifyToken param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}

// 取消实名认证
- (void)userCancelRealNameCertifiedToView:(id)toView {
    
    WH_JXConnection* p = [self addTask:act_UserCancelRealNameCertified param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}
...
-(void)WH_getMessageWithMsgId:(int)type messageId:(NSString*)messageId toView:(id)toView{
 //   WH_JXConnection* p = [self addTask:wh_act_MsgList param:[NSString stringWithFormat:@"?pageSize=%d",WH_page_size] toView:toView];
    WH_JXConnection* p = [self addTask:wh_act_MsgAllMsglist param:[NSString stringWithFormat:@"?pageSize=%d",WH_page_size] toView:toView];
...
- (void)getAgoraInfo:(id)toView fileName:(NSString *)fileName{
    
    WH_JXConnection* p = [self addTask:act_agorainfo param:nil toView:toView];
    if(fileName.length>0){
        [p setPostValue:fileName forKey:@"channel"];
    }
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
    
}
- (void)act_purchaseVipinfo:(id)toView vipConfigId:(NSString *)vipConfigId{
    WH_JXConnection* p = [self addTask:act_purchaseViinfo param:nil toView:toView];
     [p setPostValue:vipConfigId forKey:@"vipConfigId"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}
- (void)act_vipPaymentConfigListInfo:(id)toView page:(NSString *)page{
    WH_JXConnection* p = [self addTask:act_vipPaymentConfigList param:nil toView:toView];
     [p setPostValue:page forKey:@"page"];
    [p setPostValue:@"20" forKey:@"limit"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}

// 编辑消息
- (void)WH_tigaseEditMsgWithMsgId:(NSString *)msgId type:(int)type deleteType:(int)deleteType roomJid:(NSString *)roomJid msgContent:(NSString *)msgContent toView:(id)toView {
    WH_JXConnection* p = [self addTask:wh_act_tigaseEditMsg param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:msgId forKey:@"messageId"];
    [p setPostValue:msgContent forKey:@"content"];
    [p setPostValue:[NSNumber numberWithInt:type] forKey:@"type"];
...
-(void)jpushSetToken:(NSString *)devicekey deviceId:(NSString *)deviceId toView:(id)toView{
     
    WH_JXConnection* p = [self addTask:wh_act_JPushSetRegId param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:devicekey forKey:@"devicekey"];
    [p setPostValue:@"2" forKey:@"deviceId"];
    
    [p go];
}

-(void)act_codeyzfRechargeYIS:(NSString *)money payType:(NSString *)payType toView:(id)toView{
     
    WH_JXConnection* p = [self addTask:wh_act_codeyzfRecharge param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:money forKey:@"money"];
    [p setPostValue:payType forKey:@"payType"]; //1支付宝2微信
...
//5.我的邀请列表：
- (void)WH_Apiuset_UserSelectInviteUserList:(NSString *)pageIndex toView:(id)toView
{
    WH_JXConnection *p = [self addTask:act_UserSelectInviteUserList param:nil toView:toView];
    [p setPostValue:self.access_token forKey:@"access_token"];
    [p setPostValue:pageIndex forKey:@"pageIndex"];
    [p setPostValue:@"15" forKey:@"pageSize"];
    [p go];
...
//6.兑换vip消耗积分数量 ：配置config 接口获取字段 ： exchangeVipScore

//7.兑换积分接口
- (void)WH_Apiuset_UserexchangeVip:(NSString *)pageIndex toView:(id)toView
{
    WH_JXConnection *p = [self addTask:act_UserexchangeVip param:nil toView:toView];
    [p setPostValue:self.access_token forKey:@"access_token"];
    [p go];
}
//8.vip权益列表
- (void)WH_Apia_UservipConfig:(NSString *)pageIndex toView:(id)toView
{
    WH_JXConnection *p = [self addTask:act_UservipConfig param:nil toView:toView];
    [p setPostValue:self.access_token forKey:@"access_token"];
    [p go];
}
/*
...
- (NSString *)decryptAES:(NSString *)content{
   ...
   if (cryptStatus == kCCSuccess) {
       ...
       NSData *strData =  [NSData dataWithBytesNoCopy:decryptedBytes length:actualOutSize];
...
+ (NSString *)base64String:(NSString*)sourceString {//判断字符串的长度是否小于等于0如果是，就返回空
