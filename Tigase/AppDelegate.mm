...
//    NSString* defApiUrl = @"http://203.86.238.168:8092/config";//演示
    // 网络监听
...
//    [NSThread sleepForTimeInterval:0.3];
    ...
    [JPUSHService setupWithOption:launchOptions appKey:@"1fd5fc09181a454fe87f8607"
//     43c25ff15f21a8d4ca573494 通用
     //@"d23a3b0d20d60a6ed1f6e0e2"
                          channel:@"App Store"
...
- (NSString * BLY_NULLABLE)attachmentForException:(NSException * BLY_NULLABLE)exception{
    ...
    NSString        *reason= exception.name;;
    ...
    msg.toUserId     = @"10000";
    msg.isGroup = NO;
    NSString *excetiopStr = [NSString stringWithFormat:@"%@-%@-%@",name,reason,[userInfo mj_JSONString]];
    msg.content      =excetiopStr;
    ...
    msg.isReadDel = [NSNumber numberWithBool:NO];
...
- (AVAudioPlayer*)player {
    ...
    if (!_player) {
        ...
        NSURL*fileURL= [[NSBundle mainBundle] URLForResource:@"Silence" withExtension:@"wav"];
...
- (UIAlertView *) showAlert: (NSString *) message delegate:(id)delegate tag:(NSUInteger)tag sureButtonTitle:(NSString *)sureok onlyConfirm:(BOOL)onlyConfirm
{
    UIAlertView *av;
    if (onlyConfirm)
       av = [[UIAlertView alloc] initWithTitle:APP_NAME message:message delegate:delegate cancelButtonTitle:sureok otherButtonTitles:nil];
    else
        av = [[UIAlertView alloc] initWithTitle:APP_NAME message:message delegate:delegate cancelButtonTitle:Localized(@"JX_Cencal") otherButtonTitles:sureok, nil];
    av.tag = tag;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [av show];
    });
...
//    }
    ...
    [g_default setObject:tokenStr forKey:@"apnsToken"];
    [g_default synchronize];
...
- (void)getScreenShot:(NSNotification *)notification{
    ...
    BOOL isSuccess = [imageData writeToFile:NSHomeDirectory() atomically:YES];
    if (isSuccess) {
