...
#import "TMineNoteBookGaoJiVc.h"
#import "IQTextView.h"
#import "RITLPhotosViewController.h"
#import "WH_JXCamera_WHVC.h"
#import "WH_JXMediaObject.h"

@interface TMineNoteBookGaoJiVc () <UITextViewDelegate,CLLocationManagerDelegate,RITLPhotosViewControllerDelegate,LXActionSheetDelegate,WH_JXActionSheet_WHVCDelegate,WH_JXCamera_WHVCDelegate>
{
...
@implementation TMineNoteBookGaoJiVc
- (instancetype)init
{
    self = [super init];
    if (self) {
...
-(void)publishButtonItemClicked {
    ...
    if (_imageStrings.count > 0||_imageStrings.count > 0) {
        for (NSString *urllocalStr in _imageStrings) {
            [g_server uploadFile:urllocalStr validTime:@"-1" messageId:MY_USER_ID toView:self];
        }
        
        for (NSString *urllocalStr in _videoStrings) {
            [g_server uploadFile:urllocalStr validTime:@"-1" messageId:MY_USER_ID toView:self];
        }
    } else {
        
         [g_server WH_getact_NoteAddConfigUserId:_images body:self.mContentTV.text title:_title_tf.text toView:self];
    }
...
-(void)showVideos{
     int i;
    _photoIndex = -100;
    [g_factory removeAllChild:_svVideos];
     
    
    _svVideos = [[UIScrollView alloc] initWithFrame:CGRectMake(10, 0, JX_SCREEN_WIDTH-50,110)];
    _svVideos.pagingEnabled = YES;
    _svVideos.delegate = self;
    _svVideos.showsVerticalScrollIndicator = NO;
    _svVideos.showsHorizontalScrollIndicator = NO;
    _svVideos.backgroundColor = [UIColor brownColor];
    _svVideos.userInteractionEnabled = YES;
    [_videoView addSubview:_svVideos];
    [_svVideos mas_makeConstraints:^(MASConstraintMaker *make) {
        make.edges.mas_equalTo(0);
    }];
   
    NSInteger n = [_videosArr count];
    NSMutableArray *tempArr = [NSMutableArray array];
    if(_isEdit){
        NSDictionary *dict = [_dictData objectForKey:@"body"];
        for (NSDictionary *dictObj in dict[@"images"]) {
            if([dictObj[@"oFileName"] hasSuffix:@".mp4"]){
                [tempArr addObject:dictObj[@"oUrl"]];
            }
        }
        n = tempArr.count;
    }
    CGFloat width = (_svVideos.width - 20)/3;
    _svVideos.contentSize = CGSizeMake( n * (width + 5),110);
    for(i=0;i<n&&i<9;i++){
...
//        iv.didTouch = @selector(onDelVideo);
        ...
        if(_isEdit){
            ...
            iv.image = [_videosArr objectAtIndex:i];
        }
        [_svVideos addSubview:iv];
    }
    if (n == 9) {
        return;
    }
    
    _VideoViewArray = [NSMutableArray arrayWithArray:_svVideos.subviews];
    _svVideos.contentSize = CGSizeMake( n * (width + 5)+width+5,110);
    //添加图片
    UIButton *addImageBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [addImageBtn setBackgroundImage:[UIImage imageNamed:@"newicon_add_video"] forState:UIControlStateNormal];
    [_svVideos addSubview:addImageBtn];
    addImageBtn.frame = CGRectMake(i* (width +5), 5, width, width);
    addImageBtn.tag = video_tag;
    [addImageBtn addTarget:self action:@selector(donone) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setupUIEdit {
   ...
   [title_tf mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    videoView.image =[UIImage imageNamed: @"hx_videoplay"];
    videoView.userInteractionEnabled = YES;
    videoView.contentMode = UIViewContentModeScaleAspectFit;
    videoView.backgroundColor = [UIColor whiteColor];
  //  [videoView addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(imageTapVideoclickplay)]];
    ...
    NSArray *videoUrlImgs = [_dictData objectForKey:@"videoUrls"];
    if(videoUrlImgs.count){
     
        if (self.playerLayer != nil) {
            [self.playerLayer removeFromSuperlayer];
        }
        NSDictionary *modelx = videoUrlImgs.firstObject;
...
//    }];
   ...
   // [videoView addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(imageTapVideoclick)]];
    [self.view addSubview:videoView];
    _videoView = videoView;
    [videoView mas_makeConstraints:^(MASConstraintMaker *make) {
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
   [_wait stop];
    ...
    /**
     {"oFileName":"bb36cecc41684080b179dc226bf3c2af.mp4","oUrl":"http://82.157.30.116:8089/u/9/10000009/202402/bb36cecc41684080b179dc226bf3c2af.mp4","status":1}
    ...
    if([aDownload.action isEqualToString:wh_act_UploadFile]){
        ...
        NSLog(@"和 == %d",_imageStrings.count+_videoStrings.count);
        NSLog(@"和2 ==%d",_imageVideoStrings);
        
        
        if (_imageVideoStrings.count==(_imageStrings.count+_videoStrings.count)) {
