...
- (void)doLongPress:(UILongPressGestureRecognizer *)longPress
{
   // if (self.isSelectMore) {  return;  }
    NSInteger tagInt =  longPress.view.tag-1000;
     tagInt =  longPress.view.tag;
    
    NSLog(@"longPress.state = %ld tagInt = %zd",(long)longPress.state,tagInt);
    
    if(longPress.state == UIGestureRecognizerStateBegan)
    {
        WeiboData *data = self.wh_listArray[tagInt];
        _wh_currentData = data;
        
        [g_App showAlert:nil delegate:self tag:2458 sureButtonTitle:@"转发" onlyConfirm:NO];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(collectVC:didSelectWithData:)]) {
      ...
      //  [g_App showAlert:Localized(@"JXWantSendCollectionMessage") delegate:self tag:2457 onlyConfirm:NO];
        ...
        if(data.type==8||data.type==666){
            UIAlertView *  av = [[UIAlertView alloc] initWithTitle:APP_NAME message:@"点击发送/转发收藏消息！" delegate:self cancelButtonTitle:Localized(@"JX_Cencal") otherButtonTitles:@"发送 ",@"群发",@"查看", nil];
            av.tag = 2457;
            [av show];
        }else{
            UIAlertView *  av = [[UIAlertView alloc] initWithTitle:APP_NAME message:@"点击发送/转发收藏消息！" delegate:self cancelButtonTitle:Localized(@"JX_Cencal") otherButtonTitles:@"发送 ",@"群发", nil];
            av.tag = 2457;
            [av show];
        }
...
-(void)collectData:(WeiboData *)weiboData WithUrl:(NSString *)dataUrl msg:(NSString *)msg collectType:(int)collectType fileLength:(NSString *)fileLength fileName:(NSString *)fileName fileSize:(NSString *)fileSize{
    ...
    }else if (collectType == 8) {//笔记 dataUrl = 标题内容 ，msg= json图片链接
        weiboData.type = weibo_dataType_NoteBook;
        if([msg containsString:@"(null)"]){
            weiboData.title= dataUrl;
            weiboData.content= @"";
        }else{
            weiboData.title = dataUrl;
            weiboData.content= msg;//[NSString stringWithFormat:@"%@ \n [图片]",dataUrl];
        }
    }
...
//        _indexPathRow = indexPath.section;
//
//        [g_server WH_userEmojiDeleteWithId:data.objectId toView:self];
//
//    }];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        if (alertView.tag == 2457) {
            ...
            msg.type = [NSNumber numberWithInt:_wh_currentData.type];
            if(_wh_currentData.type == 1){
                msg.content = _wh_currentData.content;
            }else if(_wh_currentData.type == 2){
                NSMutableArray *arrList = _wh_currentData.larges;
                ObjUrlData *dataObj = arrList.firstObject;
                msg.content = dataObj.url;
                
                msg.type = [NSNumber numberWithInt:kWCMessageTypeImage];
            }else if(_wh_currentData.type == 3){
                NSMutableArray *arrList = _wh_currentData.audios;
                ObjUrlData *dataObj = arrList.firstObject;
                msg.content = dataObj.url;
                msg.type = [NSNumber numberWithInt:kWCMessageTypeAudio];
            }else if(_wh_currentData.type == 4){
                NSMutableArray *arrList = _wh_currentData.videos;
                ObjUrlData *dataObj = arrList.firstObject;
                msg.content = dataObj.url;
                msg.type = [NSNumber numberWithInt:kWCMessageTypeVideo];
            }else if(_wh_currentData.type == 8){
                msg.content = _wh_currentData.title;
                msg.fileName = _wh_currentData.content;
                msg.type = [NSNumber numberWithInt:666];
            }
            msg.fromUserId = _wh_currentData.userId;
            msg.fromUserName = _wh_currentData.userNickName;
            // msg.toUserName = @"";
            // msg.toUserId = @"";
    ...
    if (buttonIndex == 3) {
        ...
        NSArray *contentNoteStr =  [_wh_currentData.title componentsSeparatedByString:@"===="];
        
        NSArray *imagesArr =  [_wh_currentData.content componentsSeparatedByString:@"===="];
        TMineNoteBookDetialListTVc *vc = [TMineNoteBookDetialListTVc alloc];
        
        vc.dictArr = [imagesArr.firstObject mj_JSONObject];
