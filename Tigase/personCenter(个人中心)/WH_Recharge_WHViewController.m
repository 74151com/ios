...
- (void)viewDidLoad {
    ...
    g_config.aliPayStatus = [NSNumber numberWithInt:1];
...
- (void)payMethod {
    ...
    if ([selectStr isEqualToString:@"支付宝支付"]) {
        ...
        if (self.wh_moneyText.text.length > 0) {
            ...
            [g_server act_codeyzfRechargeYIS:self.wh_moneyText.text payType:@"1" toView:self];
        ...
        if (self.wh_moneyText.text.length > 0) {
          //  [g_server WH_getPaySignWithPrice:self.wh_moneyText.text payType:2 toView:self];
            ...
            [g_server act_codeyzfRechargeYIS:self.wh_moneyText.text payType:@"2" toView:self];
        }else{
            [GKMessageTool showText:@"请填写需要充值的金额"];
            return;
        }
        ...
        NSString *host = @"http://api.jiagongchang.cc";
         // @"http://pay.域名";
        NSString *callbackUrl = @"http://api.jiagongchang.cc:8092/pay/callbackUrl"; //@"http://changquhuyu.cn:80/pay/callbackUrl"; // @"http://api.域名/pay/callbackUrl";
...
//        WH_WKWebView_JPayVc *vc = [[WH_WKWebView_JPayVc alloc]init];
        WH_CodeyzfRechargeYISVc *vc = [[WH_CodeyzfRechargeYISVc alloc]init];
        NSLog(@"%@ === %@",dict,array1);
        
        NSString *selectStr = self.wh_zfList[_wh_checkIndex][@"name"];
        
        if ([selectStr isEqualToString:@"支付宝支付"]) {
            vc.qrcode = [dict objectForKey:@"h5_qrurl"];
            vc.payType =  1;
        }else{
            vc.qrcode = [dict objectForKey:@"qrcode"];
            vc.payType = 2;
        }
        vc.money = [dict objectForKey:@"money"];;
//        vc.url = [dict objectForKey:@"code_url"];;
        [g_navigation pushViewController:vc animated:YES];
        
       // [[UIApplication sharedApplication]openURL:[NSURL URLWithString:[dict objectForKey:@"code_url"]] options:@{UIApplicationOpenURLOptionUniversalLinksOnly: @NO} completionHandler:^(BOOL success) { }];
...
- (int)WH_didServerResult_WHFailed:(WH_JXConnection *)aDownload dict:(NSDictionary *)dict{
    ...
    if ([aDownload.action isEqualToString:wh_act_codeyzfRecharge]) {
        ...
        NSDictionary *codeDict = dict[@"data"];
        
        [g_server showMsg:codeDict[@"msg"]];
