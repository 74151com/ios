...
- (void)buttonClickMethod:(UIButton *)button {
    ...
    if ([actionTitle isEqualToString:Localized(@"JXLiveVC_Recharge")]) {
        ...
        if ([g_config.aliPayStatus integerValue] != 1 && [g_config.wechatPayStatus integerValue] != 1 && [g_config.yunPayStatus integerValue] != 1) {
            //aliPayStatus;  //支付宝充值状态 1:开启 2：关闭 wechatWithdrawStatus; //微信提现状态1：开启 2：关闭act_codeyzfRechargeYIS
         WH_Recharge_WHViewController *rechargeVC = [[WH_Recharge_WHViewController alloc] init];
          ...
          //  [GKMessageTool showText:@"暂未开放请联系客服！"];
            return;
...
//            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提现" message:@"请选择提现的方式" preferredStyle:UIAlertControllerStyleAlert];
//            // 在“好的”按钮按下时，我们让程序读取文本框中的值。
//
//            UIAlertAction *cancle = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
//                 }];
//            //TFJunYou_desiPageVc
//            UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"支付宝" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
//                WH_AddWithdrawalAccount_WalletVc  *withdrawalBGVC = [[WH_AddWithdrawalAccount_WalletVc alloc] init];
//                [g_navigation pushViewController:withdrawalBGVC animated:YES];
//
//            }];
//
//            UIAlertAction *bankAction = [UIAlertAction actionWithTitle:@"银行卡" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
//                WH_AddWithdrawa_BankVc  *withdrawalBGVC = [[WH_AddWithdrawa_BankVc alloc] init];
//
//
//                [g_navigation pushViewController:withdrawalBGVC animated:YES];
//
//            }];
//
//            [alertController addAction:bankAction];
