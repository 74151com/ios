...
@property (nonatomic ,weak) JXTopVipCellView *topVipView ;
...
//    }
    if ([g_App.isShowRedPacket integerValue] == 1) {
        self.wh_dataArray = @[@[@""] ,@[Localized(@"JX_MyBalance") ,@"分享邀请码" ,@"邀请联系人",@"笔记",Localized(@"JX_MyCollection") , Localized(@"WaHu_LifeStatus_WaHu")] ,@[Localized(@"JX_SecuritySettings") ,Localized(@"JX_PrivacySettings") ,Localized(@"WaHu_OtherSetting_WaHu")]];
        self.wh_imgsArray = @[@[@""] ,@[@"MyWallet" ,@"yaoqingma",@"jifen01" ,@"笔记"  ,@"MyCollection" ,@"My_DongTai"] ,@[@"My_AnQuanSheZhi" ,@"My_YinSiSheZhi" ,@"My_QiTaSheZhi"]];
    }else {
        //隐藏钱包
        self.wh_dataArray = @[@[@""] ,@[@"分享邀请码" ,@"邀请联系人",@"笔记",Localized(@"JX_MyCollection") , Localized(@"WaHu_LifeStatus_WaHu")] ,@[Localized(@"JX_SecuritySettings") ,Localized(@"JX_PrivacySettings") ,Localized(@"WaHu_OtherSetting_WaHu")]];
        self.wh_imgsArray = @[@[@""] ,@[@"yaoqingma",@"jifen01" ,@"笔记"  ,@"MyCollection" ,@"My_DongTai"] ,@[@"My_AnQuanSheZhi" ,@"My_YinSiSheZhi" ,@"My_QiTaSheZhi"]];
        ...
        switch (status) {
            case HttpRequestSuccess:
            {
                ...
                if([g_myself.vip intValue]>0){
                    self.wh_VipLabel.hidden = NO;
                    _topVipView.vipOpen = [g_myself.vip intValue];
                }else{
                   self.wh_VipLabel.hidden = YES;
                }
                self.wh_nameLabel.frame = CGRectMake(self.wh_headImgView.frame.origin.x + 12 + CGRectGetWidth(self.wh_headImgView.frame), 16, [self getWidthWithText:g_myself.userNickname font:15], 20);
                self.wh_VipLabel.frame = CGRectMake(self.wh_nameLabel.frame.origin.x + 12 + CGRectGetWidth(self.wh_nameLabel.frame), 16, 36 , 20);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        UIView *btnVipView = [[UIView alloc]init];
        [btnVipView setFrame:CGRectMake(0, 0, CGRectGetWidth(self.wh_tableView.frame), 144)];
//        btnVipView.backgroundColor = THEMEBACKCOLOR;
        btnVipView.userInteractionEnabled = YES;
        [btnVipView setTag:indexPath.section];
        [cell addSubview:btnVipView];
        
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn setFrame:CGRectMake(0, 0, CGRectGetWidth(self.wh_tableView.frame), 80)];
        [btn setTag:indexPath.section];
        [btnVipView addSubview:btn];
        ...
        self.wh_VipLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.wh_nameLabel.frame.origin.x + 12 + CGRectGetWidth(self.wh_nameLabel.frame), 16, 36 , 20)];
        self.wh_VipLabel.layer.cornerRadius = 8;
        self.wh_VipLabel.layer.masksToBounds = YES;
        self.wh_VipLabel.text = @"vip";
        self.wh_VipLabel.textAlignment =NSTextAlignmentCenter;
        [self.wh_VipLabel setBackgroundColor:[UIColor orangeColor]];
        [self.wh_VipLabel setTextColor:[UIColor whiteColor]];
        [self.wh_VipLabel setFont:[UIFont fontWithName:@"PingFangSC-Medium" size: 12]];
        //f[btn addSubview:self.wh_VipLabel];
        
        //用户ID
        self.userId = [[UILabel alloc] initWithFrame:CGRectMake(self.wh_headImgView.frame.origin.x + 12 + CGRectGetWidth(self.wh_headImgView.frame), 44, CGRectGetWidth(self.wh_tableView.frame) - self.wh_headImgView.frame.origin.x - CGRectGetWidth(self.wh_headImgView.frame) - 12 - 60, 20)];
        ...
        JXTopVipCellView *topVipView = [[JXTopVipCellView alloc]init];
        [topVipView setFrame:CGRectMake(0, 84, CGRectGetWidth(self.wh_tableView.frame), 64)];
        //topVipView.backgroundColor = THEMEBACKCOLOR;
        [btnVipView addSubview:topVipView];
        _topVipView = topVipView;
        topVipView.vipblcok = ^(NSString *nameStr, NSString *numStr) {
            
            TMineVipViewController *collectVC = [[TMineVipViewController alloc] init];
        ...
        for (int i = 0; i < nameArray.count; i++) {
            ...
            if (indexPath.section == 1) {
               ...
               // [btn setTitle:nameArray[indexPath.section + i + (([g_App.isShowRedPacket integerValue] == 1)?3:2)] forState:UIControlStateNormal];
            }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.wh_tableView.frame), 1)];
    [view setBackgroundColor:self.wh_tableBody.backgroundColor];
  //  [view setBackgroundColor:RGB(222, 222, 222)];
    return view;
}
...
//    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.wh_tableView.frame), 0)];
// //   [view setBackgroundColor:self.wh_tableBody.backgroundColor];
//    [view setBackgroundColor:THE_LINE_COLOR];
//    return view;
//}
...
- (void)buttonClick:(UIButton *)button {
    ...
    NSLog(@"我的 click button  :%@" ,button.titleLabel.text);
    if (button.tag == 0) {
        //个人信息
//        [g_server getUser:MY_USER_ID toView:self];
        ...
        if ([g_App.isShowRedPacket integerValue] == 1) {
            if ([button.titleLabel.text isEqualToString:@"我的钱包"]) {
                //我的钱包
            ...
            }else if ([button.titleLabel.text isEqualToString:@"会员中心"]) {
                //我的收藏
                
                
                TMineVipViewController *collectVC = [[TMineVipViewController alloc] init];
                [g_navigation pushViewController:collectVC animated:YES];
               // JXVipOpenSystemVC *collectVC = [[JXVipOpenSystemVC alloc] init];
                 
                
            }else if ([button.titleLabel.text isEqualToString:@"分享邀请码"]) {
                
                WH_ShareInvtCodeViewController *qrVC = [[WH_ShareInvtCodeViewController alloc] init];
                qrVC.wh_userId = g_myself.userId;
                qrVC.wh_nickName = g_myself.userNickname;
            ...
            }else if ([button.titleLabel.text isEqualToString:@"笔记"]) {
...
//                [self presentViewController:qrVC animated:YES completion:nil];
            ...
            }else if ([button.titleLabel.text isEqualToString:@"邀请联系人"]) {
               
                
                TATeamMeViewController *qrVC = [[TATeamMeViewController alloc] init];
            ...
            }else if ([button.titleLabel.text isEqualToString:@"生活动态"]) {
                //生活状态
...
- (CGFloat)getWidthWithText:(NSString *)text font:(CGFloat)font{
    ...
    CGRect rect = [text boundingRectWithSize:CGSizeMake(MAXFLOAT, 20) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont fontWithName:@"PingFangSC-Medium" size: font]} context:nil];
...
-(void) WH_didServerResult_WHSucces:(WH_JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1 {
    ...
    if( [aDownload.action isEqualToString:wh_act_resumeList] ){
        ...
        NSLog(@"g_myself:%@" ,g_myself);
        if([[dict objectForKey:@"vip"] intValue]>0){
            self.wh_VipLabel.hidden = NO;
            _topVipView.vipOpen = NO;// [g_myself.vip intValue];
            [_topVipView setUserData:dict];
        }else{
            [_topVipView setUserData:dict];
           self.wh_VipLabel.hidden = YES;
        }
        ...
        self.userId.text = @"";
        self.userId.text = [NSString stringWithFormat:@"账号:%@",g_myself.account?:@""];
         int hasRealNameAuth= [[dict objectForKey:@"checkRealNameAuthStatus"] intValue];
  
        if(_iscerName == YES){
            _iscerName = NO;
            
            if(hasRealNameAuth==1){
                [g_server showMsg:@"正在审核"];
                
            }else if(hasRealNameAuth==2){
                [g_server showMsg:@"已认证"];
            } else if(hasRealNameAuth==3){
                [g_server showMsg:@"被拒绝请重新提交认证"];
                // startActivity(new Intent(getActivity(), RealNameActivity.class));
                JXCertificationVC *collectVC = [[JXCertificationVC alloc] init];
                collectVC.certificeSuccess = ^{
