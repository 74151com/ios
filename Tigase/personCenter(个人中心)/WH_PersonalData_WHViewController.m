...
//        tStr = Localized(@"WaHu_UserName_WaHu");
    ...
    if ([g_config.registerInviteCode intValue] == 2||[g_config.registerInviteCode intValue] == 1||[g_config.registerInviteCode intValue] == 0) {//注册邀请码   0：关闭,1:开启一对一邀请（一码一用，且必填），2:开启一对多邀请（一码多用，选填项）
...
//        }
        ...
        for (int i = 0; i < nameArray.count; i++) {
            ...
            if ([currentTitleString isEqualToString:@"手机号"]) {
                ...
                btn.tag = 103;
                //邀请码
                [btn addSubview:[self createLabelWithWidth:self.wh_cTableView.frame.size.width - 90 -12 text:g_myself.serInviteCode?:@""]];
            }
        }
    }else{
